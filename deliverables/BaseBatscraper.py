...
entry = []
with open('BaseBatstats.csv', mode="w") as f:
    writer = csv.writer(f)
    writer.writerow(['stat_type','stat_number','BSathleteID','teamID'])
...
##            entry.append(last)
        ...
        for line in stats:
            ...
            if length > 40 and length < 60:
                ...
                #print(len(stat_ty), stat_ty)
##                print(statType + ": " + statNum)
                if len(stat_ty) < 20:
                    stat_type = stat_ty
                    #print(stat_type)
                    if stat_type == 'GP-GS' or stat_type == 'SB':
                        #print(stat_type)
                        dash = stat_type.find('-')

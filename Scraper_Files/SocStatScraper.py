...
individual = soup.find('section', id='individual-overall-offensive')
individual_stat = individual.find('tbody')

entry = []
with open('SocStats.csv', mode="w") as f:
    writer = csv.writer(f)
...
##                print(statType + ": " + statNum)
                if stat_type == 'GP-GS' or stat_type == 'YC-RC' or stat_type == 'PG-PA':
                    dash = stat_type.find('-')
